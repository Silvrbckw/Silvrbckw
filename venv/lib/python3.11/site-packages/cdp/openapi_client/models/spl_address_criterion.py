...
class SplAddressCriterion(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="The type of criterion to use. This should be `splAddress`.")
    addresses: List[Annotated[str, Field(strict=True)]] = Field(description="The Solana addresses that are compared to the list of SPL token transfer recipient addresses in the transaction's `accountKeys` (for legacy transactions) or `staticAccountKeys` (for V0 transactions) array.")
    operator: StrictStr = Field(description="The operator to use for the comparison. Each of the SPL token transfer recipient addresses in the transaction's `accountKeys` (for legacy transactions) or `staticAccountKeys` (for V0 transactions) array will be on the left-hand side of the operator, and the `addresses` field will be on the right-hand side.")
    __properties: ClassVar[List[str]] = ["type", "addresses", "operator"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['splAddress']):
            raise ValueError("must be one of enum values ('splAddress')")
        return value
